...
**鉴权**：登录成功返回 `access_token`，后续请求头带 `Authorization: Bearer <access_token>`。
**没有 token 刷新机制**：收到 401 即视为登录失效，让用户重新登录。
...
- 单端登录：同一账号在别处登录会顶掉本端，本端下一次请求收到 401，需重新登录。
- access_token 有效期 7 天（服务端配置），前端无需关心，只需处理 401。
