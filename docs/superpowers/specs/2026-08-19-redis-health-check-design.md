...
健康检查异常时必须输出专用告警日志，至少包含健康检查接口路径、失败的依赖名称、检查结果和请求标识；日志使用项目现有 Pino Logger，异常对象作为结构化字段记录，不打印密码、连接字符串或完整内部堆栈到接口响应。Redis 和数据库同时异常时应在同一条健康检查异常日志中汇总两项失败状态，避免仅记录其中一项。

成功和失败响应以 Terminus 标准结构为基础，具体字段由所使用的 `@nestjs/terminus` 版本确定；项目只补充必要的统一响应包装，不重复实现依赖状态汇总。
